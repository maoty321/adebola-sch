...
if(isset($_POST['admin_login'])) {
    ...
    $select_user = "SELECT * FROM `admin_school` WHERE email = '$email'";
    $result = mysqli_query($conn, $select_user);
    if(mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        if(password_verify($password, $row["password"])) { 
            $_SESSION['alertMsg'] = true;
            $_SESSION['admin_fcaf'] = true;
            $_SESSION['admin_user_id'] = $row['admin_id'];
            $_SESSION['text'] = 'Welcome Back '.$row['name'];
        ...
        <div class="verify-box col-md-5 p-4 col-sm-12">
            ...
            <form action="" method="post">
                <div class="verify-form">
                    ...
                    <div class="mb-3">
                        <input type="submit" value="Login" name="admin_login" class="btn "><br>
                    </div>
